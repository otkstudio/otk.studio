...
const PostLink = ({
  ...
  useEffect(() => {
    const height = projectContainer.current.scrollHeight;
    setTimeout(() => setContentHeight(height), 400);
  ...
  return (
    ...
    >
      ...
      >
        <GridItem>
          <InlineContainer>{project.frontmatter.title} {project.frontmatter.new && <NewIndicator>New</NewIndicator>}</InlineContainer>
          <PracticeContainer>
            {project.frontmatter.practice.map(practice => 
              <PracticeIndicator key={practice} practice={practice} projectIsOpen={projectIsOpen} />
            )}
          </PracticeContainer>
        </GridItem>
...
const Container = styled.div`
  height: ${({ projectIsOpen, index, contentHeight }) => index === projectIsOpen ? contentHeight : 32}px;
  position: relative;
...
const Arrow = styled.svg`
  transform: rotate(${({ projectIsOpen, index }) => index === projectIsOpen ? 90 : 0}deg);
  background-color: ${({ projectIsOpen, index }) => index === projectIsOpen ? 'red' : '#cccccc'};
  border-radius: 100px;
  padding-top: 1px;
  padding-left: 1px;
  width: 20px;
...
const PracticeIndicator = styled.div`
  ...
  background: ${({ practice }) => {
    let color;
    if ( practice === 'Strategy') { color = '#aaaaaa' }
    if ( practice === 'Design') { color = '#ffaf00' }
    if ( practice === 'Development') { color = '#ff0000' }
    if ( practice === 'Art Direction') { color = '#0092ff' }
    if ( practice === 'Photography') { color = '#05e900' }
    return color;
  }};
  margin-left: 2px;
`;
